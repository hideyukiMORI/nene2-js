...
content-type variant 4. Handler `bulk_ct_4` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT483"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT483 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_4` completed under Vitest.
