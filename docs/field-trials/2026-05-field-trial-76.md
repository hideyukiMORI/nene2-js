...
Authorization Bearer. Handler `hdr_bearer_fmt` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT76"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT76 | 1     | PASS (automated) |
...
- Scenario `hdr_bearer_fmt` completed under Vitest.
- Persona A path: RFC format
