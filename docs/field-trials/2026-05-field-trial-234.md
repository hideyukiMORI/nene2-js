# FT234: problem HTTP 403 on getNote
...
problem HTTP 403 on getNote. Handler `bulk_err_403` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT234"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT234 | 1     | PASS (automated) |
...
- Scenario `bulk_err_403` completed under Vitest.
- Persona A path: Error surface 403
