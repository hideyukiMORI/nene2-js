# FT232: problem HTTP 401 on getNote
...
problem HTTP 401 on getNote. Handler `bulk_err_401` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT232"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT232 | 1     | PASS (automated) |
...
- Scenario `bulk_err_401` completed under Vitest.
- Persona A path: Error surface 401
