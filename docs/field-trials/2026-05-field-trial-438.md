...
auth header combo 3. Handler `bulk_hdr_variant_3` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT438"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT438 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_3` completed under Vitest.
