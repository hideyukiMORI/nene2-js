# FT230: problem HTTP 400 on getNote
...
problem HTTP 400 on getNote. Handler `bulk_err_400` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT230"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT230 | 1     | PASS (automated) |
...
- Scenario `bulk_err_400` completed under Vitest.
- Persona A path: Error surface 400
