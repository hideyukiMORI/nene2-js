# FT140: [ts_app/alt_ports] ja doc: getting_started_curl_health
...
[ts_app/alt_ports] ja doc: getting_started_curl_health. Handler `onb_doc_ts_app_alt_ports_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `onb_doc_ts_app_alt_ports_getting_started_curl_health`                                 |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT140"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT140 | 1     | PASS (automated) |
...
- Scenario `onb_doc_ts_app_alt_ports_getting_started_curl_health` completed under Vitest.
- Persona A path: TypeScript アプリ開発者（知識ゼロ）
- Persona B path: hideyukimori.github.io/nene2-js/ja を初めて読む / evac :18080 / python :8000 の切替
...
シナリオ `onb_doc_ts_app_alt_ports_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
