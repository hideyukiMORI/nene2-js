...
isProblemDetails. Handler `prob_is_pd` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT97"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT97 | 1     | PASS (automated) |
