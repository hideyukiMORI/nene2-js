# FT241: problem HTTP 409 on createNote
...
problem HTTP 409 on createNote. Handler `bulk_err_409_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT241"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT241 | 1     | PASS (automated) |
...
- Scenario `bulk_err_409_create` completed under Vitest.
- Persona A path: POST failure 409
