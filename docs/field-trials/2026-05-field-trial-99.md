...
parseResponse. Handler `prob_parse_resp` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT99"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT99 | 1 | PASS (automated) |
...
- Scenario `prob_parse_resp` completed under Vitest.
- Persona A path: Raw fetch
