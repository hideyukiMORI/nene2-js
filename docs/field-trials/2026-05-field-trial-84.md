...
fetch required. Handler `cfg_no_fetch` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT84"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT84 | 1 | PASS (automated) |
...
- Scenario `cfg_no_fetch` completed under Vitest.
- Persona A path: Node 22
