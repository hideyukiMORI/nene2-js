...
parallel calls batch 11. Handler `bulk_conc_11` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT461"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT461 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_11` completed under Vitest.
- Persona A path: Concurrent client usage
...
### O1: 自動シナリオ `bulk_conc_11`

カタログ `concurrency-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
