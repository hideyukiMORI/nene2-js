...
content-type variant 0. Handler `bulk_ct_0` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT479"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT479 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_0` completed under Vitest.
