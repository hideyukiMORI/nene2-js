...
replay ping_ok #11. Handler `ping_ok` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT498"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT498 | 1     | PASS (automated) |
...
- Scenario `ping_ok` completed under Vitest.
