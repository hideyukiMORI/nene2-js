...
ping pong. Handler `ping_ok` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT39"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT39 | 1 | PASS (automated) |
...
- Persona A path: Quick alive check
