...
parallel calls batch 9. Handler `bulk_conc_9` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT459"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT459 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_9` completed under Vitest.
