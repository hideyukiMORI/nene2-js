# FT238: problem HTTP 405 on getNote
...
problem HTTP 405 on getNote. Handler `bulk_err_405` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT238"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT238 | 1     | PASS (automated) |
...
- Scenario `bulk_err_405` completed under Vitest.
- Persona A path: Error surface 405
