# FT243: problem HTTP 413 on createNote
...
problem HTTP 413 on createNote. Handler `bulk_err_413_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT243"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT243 | 1     | PASS (automated) |
...
- Scenario `bulk_err_413_create` completed under Vitest.
- Persona A path: POST failure 413
