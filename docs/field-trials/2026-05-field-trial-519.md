...
misc client probe 2. Handler `bulk_misc_2` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT519"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT519 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_2` completed under Vitest.
