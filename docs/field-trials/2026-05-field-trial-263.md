...
tags list limit=5. Handler `bulk_q_tags_limit_5` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT263"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT263 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_5` completed under Vitest.
