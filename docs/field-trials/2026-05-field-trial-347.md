# FT347: tags limit+offset combo 17
...
tags limit+offset combo 17. Handler `bulk_q_tags_combo_17` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT347"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT347 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_17` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_combo_17`
