...
deleteTag. Handler `tags_delete` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT61"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT61 | 1     | PASS (automated) |
...
- Scenario `tags_delete` completed under Vitest.
- Persona A path: Remove
