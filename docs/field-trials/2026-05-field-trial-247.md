# FT247: problem HTTP 429 on createNote
...
problem HTTP 429 on createNote. Handler `bulk_err_429_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT247"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT247 | 1     | PASS (automated) |
...
- Scenario `bulk_err_429_create` completed under Vitest.
- Persona A path: POST failure 429
