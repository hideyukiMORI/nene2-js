...
tags list limit=1. Handler `bulk_q_tags_limit_1` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT257"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT257 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_1` completed under Vitest.
