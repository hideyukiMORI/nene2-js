...
update path. Handler `tags_update_path` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT64"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT64 | 1 | PASS (automated) |
...
- Scenario `tags_update_path` completed under Vitest.
- Persona A path: PUT path
