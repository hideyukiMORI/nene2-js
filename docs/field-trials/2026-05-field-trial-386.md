# FT386: validation field index 1
...
validation field index 1. Handler `bulk_vdx_fields_1` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT386"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT386 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_1` completed under Vitest.
