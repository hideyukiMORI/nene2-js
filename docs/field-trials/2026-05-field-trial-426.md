# FT426: parseProblemDetails case 16
...
parseProblemDetails case 16. Handler `bulk_prob_parse_16` (fixture, category `problem-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT426"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT426 | 1     | PASS (automated) |
...
- Scenario `bulk_prob_parse_16` completed under Vitest.
- Persona A path: Problem parsers
...
### O1: 自動シナリオ `bulk_prob_parse_16`

カタログ `problem-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
