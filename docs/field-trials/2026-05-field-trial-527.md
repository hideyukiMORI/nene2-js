...
misc client probe 10. Handler `bulk_misc_10` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT527"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT527 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_10` completed under Vitest.
- Persona A path: Misc DX
...
### O1: 自動シナリオ `bulk_misc_10`

カタログ `misc-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
