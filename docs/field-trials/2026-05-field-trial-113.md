...
create then get. Handler `conc_create_get` (fixture, category `concurrency`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT113"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT113 | 1 | PASS (automated) |
...
- Scenario `conc_create_get` completed under Vitest.
- Persona A path: Optimistic UI
...
### O1: 自動シナリオ `conc_create_get`

カタログ `concurrency` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
