...
misc client probe 4. Handler `bulk_misc_4` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT521"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT521 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_4` completed under Vitest.
