...
replay smoke_ok #22. Handler `smoke_ok` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT509"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT509 | 1     | PASS (automated) |
...
- Scenario `smoke_ok` completed under Vitest.
