...
updateNote. Handler `notes_update` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT49"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT49 | 1     | PASS (automated) |
...
- Scenario `notes_update` completed under Vitest.
- Persona A path: Edit save
