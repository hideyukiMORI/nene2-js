...
getTag id=55. Handler `bulk_tags_id_55` (fixture, category `tags-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT382"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT382 | 1     | PASS (automated) |
...
- Scenario `bulk_tags_id_55` completed under Vitest.
