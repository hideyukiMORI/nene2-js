...
parallel calls batch 1. Handler `bulk_conc_1` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT451"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT451 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_1` completed under Vitest.
