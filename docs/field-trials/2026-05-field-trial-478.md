...
fixture load tag-ok. Handler `bulk_fixture_tag_ok` (fixture, category `fixtures-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT478"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT478 | 1     | PASS (automated) |
...
- Scenario `bulk_fixture_tag_ok` completed under Vitest.
