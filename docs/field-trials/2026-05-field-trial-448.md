...
auth header combo 13. Handler `bulk_hdr_variant_13` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT448"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT448 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_13` completed under Vitest.
