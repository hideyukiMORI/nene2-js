# FT403: validation field index 18
...
validation field index 18. Handler `bulk_vdx_fields_18` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT403"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT403 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_18` completed under Vitest.
