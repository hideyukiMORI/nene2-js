...
no apiKey on health. Handler `hdr_no_key_health` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT78"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT78 | 1 | PASS (automated) |
...
- Scenario `hdr_no_key_health` completed under Vitest.
- Persona A path: Public path
...
### O1: 自動シナリオ `hdr_no_key_health`

カタログ `auth-headers` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
