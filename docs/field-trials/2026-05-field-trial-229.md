...
live onboarding 5. Handler `onb_live_notes_journey` (live, category `docs-live-onboarding`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
| Handler     | `onb_live_notes_journey`                                                               |
| Mode        | live                                                                                   |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT229"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT229 | 1     | PASS (automated) |
...
- Scenario `onb_live_notes_journey` completed under Vitest.
- Persona A path: サーバ起動後に ja howto/live-smoke を実行
- Persona B path: AGENTS.md なしで ja サイトだけ渡された想定
...
### O1: 自動シナリオ `onb_live_notes_journey`
