# FT34: health service must be NENE2
...
health service must be NENE2. Handler `health_service_nene2` (fixture, category `health`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT34"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT34 | 1     | PASS (automated) |
...
- Scenario `health_service_nene2` completed under Vitest.
- Persona A path: Detect wrong backend
...
### O1: 自動シナリオ `health_service_nene2`

カタログ `health` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
