...
getTag id=8. Handler `bulk_tags_id_8` (fixture, category `tags-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT381"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT381 | 1     | PASS (automated) |
...
- Scenario `bulk_tags_id_8` completed under Vitest.
