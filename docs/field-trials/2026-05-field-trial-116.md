...
notes both. Handler `q_notes_both` (fixture, category `query`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT116"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT116 | 1     | PASS (automated) |
...
- Scenario `q_notes_both` completed under Vitest.
- Persona A path: Pager
