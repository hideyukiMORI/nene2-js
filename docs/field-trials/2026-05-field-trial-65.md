...
delete path. Handler `tags_delete_path` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT65"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT65 | 1 | PASS (automated) |
...
- Scenario `tags_delete_path` completed under Vitest.
- Persona A path: DELETE path
