...
notes offset=0. Handler `bulk_q_notes_offset_0` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT290"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT290 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_offset_0` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_offset_0`
