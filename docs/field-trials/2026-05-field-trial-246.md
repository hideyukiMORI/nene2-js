# FT246: problem HTTP 429 on getNote
...
problem HTTP 429 on getNote. Handler `bulk_err_429` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT246"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT246 | 1     | PASS (automated) |
...
- Scenario `bulk_err_429` completed under Vitest.
- Persona A path: Error surface 429
