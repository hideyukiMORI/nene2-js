...
auth header combo 11. Handler `bulk_hdr_variant_11` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT446"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT446 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_11` completed under Vitest.
