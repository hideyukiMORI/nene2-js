# FT253: problem HTTP 503 on createNote
...
problem HTTP 503 on createNote. Handler `bulk_err_503_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT253"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT253 | 1     | PASS (automated) |
...
- Scenario `bulk_err_503_create` completed under Vitest.
- Persona A path: POST failure 503
