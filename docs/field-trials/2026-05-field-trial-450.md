...
parallel calls batch 0. Handler `bulk_conc_0` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT450"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT450 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_0` completed under Vitest.
