# FT320: notes limit+offset combo 4
...
notes limit+offset combo 4. Handler `bulk_q_notes_combo_4` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT320"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT320 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_4` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_4`
