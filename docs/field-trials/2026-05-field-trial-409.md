# FT409: validation field index 24
...
validation field index 24. Handler `bulk_vdx_fields_24` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT409"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT409 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_24` completed under Vitest.
