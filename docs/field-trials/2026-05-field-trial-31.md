...
health 503 throw default. Handler `health_503_throw` (fixture, category `health`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT31"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT31 | 1     | PASS (automated) |
...
- Scenario `health_503_throw` completed under Vitest.
- Persona A path: LB expects throw by default
