...
public no auth. Handler `hdr_public_health` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT77"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT77 | 1     | PASS (automated) |
...
- Scenario `hdr_public_health` completed under Vitest.
- Persona A path: Anonymous health
