...
tags list limit=500. Handler `bulk_q_tags_limit_500` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT287"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT287 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_500` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_limit_500`
