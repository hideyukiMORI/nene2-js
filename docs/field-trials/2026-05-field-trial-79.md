...
trailing slash. Handler `cfg_trailing_slash` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT79"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT79 | 1 | PASS (automated) |
...
- Scenario `cfg_trailing_slash` completed under Vitest.
- Persona A path: Copy-paste URL
