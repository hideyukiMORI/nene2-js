# FT173: [php_dev/alt_ports] ja doc: getting_started_auth_env
...
[php_dev/alt_ports] ja doc: getting_started_auth_env. Handler `onb_doc_php_dev_alt_ports_getting_started_auth_env` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_php_dev_alt_ports_getting_started_auth_env` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT173"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT173 | 1 | PASS (automated) |
...
- Scenario `onb_doc_php_dev_alt_ports_getting_started_auth_env` completed under Vitest.
...
シナリオ `onb_doc_php_dev_alt_ports_getting_started_auth_env` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
