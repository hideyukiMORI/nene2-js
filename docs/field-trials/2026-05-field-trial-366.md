...
deleteNote id=99. Handler `bulk_notes_del_99` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT366"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT366 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_del_99` completed under Vitest.
