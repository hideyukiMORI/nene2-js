# FT333: tags limit+offset combo 10
...
tags limit+offset combo 10. Handler `bulk_q_tags_combo_10` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT333"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT333 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_10` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_combo_10`
