# FT388: validation field index 3
...
validation field index 3. Handler `bulk_vdx_fields_3` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT388"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT388 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_3` completed under Vitest.
