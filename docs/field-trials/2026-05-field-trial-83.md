...
port in baseUrl. Handler `cfg_port_url` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT83"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT83 | 1 | PASS (automated) |
...
- Scenario `cfg_port_url` completed under Vitest.
- Persona A path: Evac 18080
