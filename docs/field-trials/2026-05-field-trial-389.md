# FT389: validation field index 4
...
validation field index 4. Handler `bulk_vdx_fields_4` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT389"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT389 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_4` completed under Vitest.
