...
parallel calls batch 8. Handler `bulk_conc_8` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT458"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT458 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_8` completed under Vitest.
