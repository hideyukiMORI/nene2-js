...
listTags query. Handler `tags_list_query` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT57"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT57 | 1     | PASS (automated) |
...
- Scenario `tags_list_query` completed under Vitest.
