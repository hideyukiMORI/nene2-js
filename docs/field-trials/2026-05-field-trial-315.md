# FT315: tags limit+offset combo 1
...
tags limit+offset combo 1. Handler `bulk_q_tags_combo_1` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT315"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT315 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_1` completed under Vitest.
