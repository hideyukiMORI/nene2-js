...
smoke parallel. Handler `conc_smoke` (fixture, category `concurrency`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT110"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT110 | 1 | PASS (automated) |
...
- Scenario `conc_smoke` completed under Vitest.
- Persona A path: Latency
