# FT213: [py_dev] client: health_wrong_service
...
[py_dev] client: health_wrong_service. Handler `onb_client_py_dev_health_wrong_service` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `onb_client_py_dev_health_wrong_service`                                               |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT213"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT213 | 1     | PASS (automated) |
...
- Scenario `onb_client_py_dev_health_wrong_service` completed under Vitest.
...
### F-1: health.service が NENE2 以外でも成功する (severity: medium)
...
### O1: `health.service` は型ガードで NENE2 限定しない
