# FT350: notes limit+offset combo 19
...
notes limit+offset combo 19. Handler `bulk_q_notes_combo_19` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT350"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT350 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_19` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_19`
