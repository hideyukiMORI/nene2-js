...
notes list limit=75. Handler `bulk_q_notes_limit_75` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT276"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT276 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_limit_75` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_limit_75`
