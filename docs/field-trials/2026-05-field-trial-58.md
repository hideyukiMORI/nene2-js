...
getTag. Handler `tags_get` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT58"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT58 | 1 | PASS (automated) |
...
- Scenario `tags_get` completed under Vitest.
- Persona A path: Detail
