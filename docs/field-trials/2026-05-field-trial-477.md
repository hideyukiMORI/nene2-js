...
fixture load note-ok. Handler `bulk_fixture_note_ok` (fixture, category `fixtures-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT477"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT477 | 1     | PASS (automated) |
...
- Scenario `bulk_fixture_note_ok` completed under Vitest.
...
### O1: 自動シナリオ `bulk_fixture_note_ok`
