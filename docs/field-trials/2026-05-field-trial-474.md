# FT474: fixture load payload-too-large
...
fixture load payload-too-large. Handler `bulk_fixture_payload_too_large` (fixture, category `fixtures-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `bulk_fixture_payload_too_large`                                                       |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT474"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT474 | 1     | PASS (automated) |
...
- Scenario `bulk_fixture_payload_too_large` completed under Vitest.
- Persona A path: Fixture JSON sanity
...
### O1: 自動シナリオ `bulk_fixture_payload_too_large`

カタログ `fixtures-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
