...
content-type variant 5. Handler `bulk_ct_5` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT484"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT484 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_5` completed under Vitest.
