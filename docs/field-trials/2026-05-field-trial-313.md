# FT313: tags limit+offset combo 0
...
tags limit+offset combo 0. Handler `bulk_q_tags_combo_0` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT313"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT313 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_0` completed under Vitest.
