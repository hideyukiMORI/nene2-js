...
401 unauthorized. Handler `err_401` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT90"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT90 | 1     | PASS (automated) |
...
- Scenario `err_401` completed under Vitest.
- Persona A path: Auth gate
