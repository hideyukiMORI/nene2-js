...
getNote path id=100. Handler `bulk_notes_id_100` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT367"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT367 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_id_100` completed under Vitest.
