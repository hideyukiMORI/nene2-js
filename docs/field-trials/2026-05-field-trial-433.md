...
baseUrl variant 3. Handler `bulk_cfg_base_3` (fixture, category `config-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT433"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT433 | 1     | PASS (automated) |
...
- Scenario `bulk_cfg_base_3` completed under Vitest.
