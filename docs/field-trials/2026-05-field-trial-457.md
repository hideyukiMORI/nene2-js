...
parallel calls batch 7. Handler `bulk_conc_7` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT457"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT457 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_7` completed under Vitest.
