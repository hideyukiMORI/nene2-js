...
deleteNote. Handler `notes_delete` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT50"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT50 | 1     | PASS (automated) |
...
- Scenario `notes_delete` completed under Vitest.
- Persona A path: Remove row
