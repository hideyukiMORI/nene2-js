...
shape mismatch. Handler `err_shape` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT96"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT96 | 1 | PASS (automated) |
...
- Scenario `err_shape` completed under Vitest.
- Persona A path: Contract drift
