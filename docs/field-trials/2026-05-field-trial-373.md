...
getNote path id=10000. Handler `bulk_notes_id_10000` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT373"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT373 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_id_10000` completed under Vitest.
