# FT385: validation field index 0
...
validation field index 0. Handler `bulk_vdx_fields_0` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT385"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT385 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_0` completed under Vitest.
