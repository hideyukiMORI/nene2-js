# FT202: [ts_app] client: smoke_fixture
...
[ts_app] client: smoke_fixture. Handler `onb_client_ts_app_smoke_fixture` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_client_ts_app_smoke_fixture` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT202"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT202 | 1 | PASS (automated) |
...
- Scenario `onb_client_ts_app_smoke_fixture` completed under Vitest.
...
### O1: 自動シナリオ `onb_client_ts_app_smoke_fixture`
