...
getTag 404. Handler `tags_get_404` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT62"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT62 | 1 | PASS (automated) |
...
- Scenario `tags_get_404` completed under Vitest.
- Persona A path: Missing
