...
seq health. Handler `conc_seq_health` (fixture, category `concurrency`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT112"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT112 | 1     | PASS (automated) |
...
- Scenario `conc_seq_health` completed under Vitest.
- Persona A path: Polling
