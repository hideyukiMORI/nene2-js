...
live smoke. Handler `live_smoke` (live, category `live-evac`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT121"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT121 | 1 | PASS (automated) |
...
- Scenario `live_smoke` completed under Vitest.
