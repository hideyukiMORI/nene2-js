# FT207: [ts_app] client: html_wrong_port_hint
...
[ts_app] client: html_wrong_port_hint. Handler `onb_client_ts_app_html_wrong_port_hint` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_client_ts_app_html_wrong_port_hint` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT207"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT207 | 1 | PASS (automated) |
...
- Scenario `onb_client_ts_app_html_wrong_port_hint` completed under Vitest.
...
### O1: 自動シナリオ `onb_client_ts_app_html_wrong_port_hint`
