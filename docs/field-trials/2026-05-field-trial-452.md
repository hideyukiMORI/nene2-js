...
parallel calls batch 2. Handler `bulk_conc_2` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT452"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT452 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_2` completed under Vitest.
