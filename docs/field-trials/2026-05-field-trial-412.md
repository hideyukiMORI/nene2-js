# FT412: parseProblemDetails case 2
...
parseProblemDetails case 2. Handler `bulk_prob_parse_2` (fixture, category `problem-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT412"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT412 | 1     | PASS (automated) |
...
- Scenario `bulk_prob_parse_2` completed under Vitest.
