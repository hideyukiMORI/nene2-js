...
getTag id=9999. Handler `bulk_tags_id_9999` (fixture, category `tags-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT384"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT384 | 1     | PASS (automated) |
...
- Scenario `bulk_tags_id_9999` completed under Vitest.
- Persona A path: Tag path id
...
### O1: 自動シナリオ `bulk_tags_id_9999`

カタログ `tags-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
