...
two clients. Handler `cfg_two_clients` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT85"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT85 | 1     | PASS (automated) |
...
- Scenario `cfg_two_clients` completed under Vitest.
- Persona A path: Multi-tenant
