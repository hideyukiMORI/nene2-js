# FT392: validation field index 7
...
validation field index 7. Handler `bulk_vdx_fields_7` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT392"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT392 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_7` completed under Vitest.
