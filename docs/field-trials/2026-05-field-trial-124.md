...
live listNotes. Handler `live_list_notes` (live, category `live-evac`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT124"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT124 | 1 | PASS (automated) |
...
- Scenario `live_list_notes` completed under Vitest.
