...
misc client probe 8. Handler `bulk_misc_8` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT525"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT525 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_8` completed under Vitest.
