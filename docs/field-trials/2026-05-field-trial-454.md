...
parallel calls batch 4. Handler `bulk_conc_4` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT454"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT454 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_4` completed under Vitest.
