# FT316: notes limit+offset combo 2
...
notes limit+offset combo 2. Handler `bulk_q_notes_combo_2` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT316"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT316 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_2` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_2`
