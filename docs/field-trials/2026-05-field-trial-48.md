...
createNote. Handler `notes_create` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT48"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT48 | 1 | PASS (automated) |
...
- Scenario `notes_create` completed under Vitest.
- Persona A path: Form submit
