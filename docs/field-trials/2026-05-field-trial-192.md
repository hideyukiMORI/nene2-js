# FT192: [devops/client_only] ja doc: getting_started_curl_health
...
[devops/client_only] ja doc: getting_started_curl_health. Handler `onb_doc_devops_client_only_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_devops_client_only_getting_started_curl_health` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT192"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT192 | 1 | PASS (automated) |
...
- Scenario `onb_doc_devops_client_only_getting_started_curl_health` completed under Vitest.
- Persona A path: DevOps / SRE（ヘルスとポートの切り分け）
- Persona B path: 8080/8000/18080 の取り違えを疑う / サーバ未起動で client のみ導入
...
シナリオ `onb_doc_devops_client_only_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
