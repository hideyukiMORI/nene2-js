...
createNote POST body. Handler `notes_create_body` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT53"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT53 | 1     | PASS (automated) |
...
- Scenario `notes_create_body` completed under Vitest.
- Persona A path: Payload inspect
...
### O1: 自動シナリオ `notes_create_body`

カタログ `notes` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
