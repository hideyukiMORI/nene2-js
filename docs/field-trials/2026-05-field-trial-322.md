# FT322: notes limit+offset combo 5
...
notes limit+offset combo 5. Handler `bulk_q_notes_combo_5` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT322"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT322 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_5` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_5`
