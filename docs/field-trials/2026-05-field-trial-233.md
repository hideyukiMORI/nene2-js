# FT233: problem HTTP 401 on createNote
...
problem HTTP 401 on createNote. Handler `bulk_err_401_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT233"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT233 | 1     | PASS (automated) |
...
- Scenario `bulk_err_401_create` completed under Vitest.
- Persona A path: POST failure 401
