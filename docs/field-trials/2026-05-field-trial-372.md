...
deleteNote id=999. Handler `bulk_notes_del_999` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT372"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT372 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_del_999` completed under Vitest.
