...
ping bad message. Handler `ping_bad` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT44"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT44 | 1 | PASS (automated) |
...
- Scenario `ping_bad` completed under Vitest.
