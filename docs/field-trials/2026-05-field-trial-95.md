...
problem detail text. Handler `err_detail` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT95"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT95 | 1     | PASS (automated) |
...
- Scenario `err_detail` completed under Vitest.
- Persona A path: Toast message
...
### O1: 自動シナリオ `err_detail`

カタログ `errors` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
