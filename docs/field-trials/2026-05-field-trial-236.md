# FT236: problem HTTP 404 on getNote
...
problem HTTP 404 on getNote. Handler `bulk_err_404` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT236"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT236 | 1     | PASS (automated) |
...
- Scenario `bulk_err_404` completed under Vitest.
- Persona A path: Error surface 404
