...
frameworkSmoke GET /. Handler `framework_ok` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT38"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT38 | 1 | PASS (automated) |
...
- Scenario `framework_ok` completed under Vitest.
- Persona A path: Root metadata
