...
parallel calls batch 3. Handler `bulk_conc_3` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT453"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT453 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_3` completed under Vitest.
