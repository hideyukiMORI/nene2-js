# FT250: problem HTTP 502 on getNote
...
problem HTTP 502 on getNote. Handler `bulk_err_502` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT250"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT250 | 1     | PASS (automated) |
...
- Scenario `bulk_err_502` completed under Vitest.
- Persona A path: Error surface 502
