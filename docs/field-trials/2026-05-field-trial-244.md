# FT244: problem HTTP 422 on getNote
...
problem HTTP 422 on getNote. Handler `bulk_err_422` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT244"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT244 | 1     | PASS (automated) |
...
- Scenario `bulk_err_422` completed under Vitest.
- Persona A path: Error surface 422
