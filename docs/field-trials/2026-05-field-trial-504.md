...
replay prob_is_pd #17. Handler `prob_is_pd` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT504"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT504 | 1     | PASS (automated) |
...
- Scenario `prob_is_pd` completed under Vitest.
