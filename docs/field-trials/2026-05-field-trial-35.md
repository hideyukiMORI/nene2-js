...
health invalid shape. Handler `health_invalid_shape` (fixture, category `health`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT35"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT35 | 1     | PASS (automated) |
...
- Scenario `health_invalid_shape` completed under Vitest.
- Persona A path: Guard typo responses
...
### O1: 自動シナリオ `health_invalid_shape`
