# FT346: notes limit+offset combo 17
...
notes limit+offset combo 17. Handler `bulk_q_notes_combo_17` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT346"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT346 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_17` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_17`
