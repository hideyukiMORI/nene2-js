# FT329: tags limit+offset combo 8
...
tags limit+offset combo 8. Handler `bulk_q_tags_combo_8` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT329"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT329 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_8` completed under Vitest.
