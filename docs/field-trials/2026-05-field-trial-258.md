...
notes list limit=2. Handler `bulk_q_notes_limit_2` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT258"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT258 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_limit_2` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_limit_2`
