...
notes offset=2. Handler `bulk_q_notes_offset_2` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT294"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT294 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_offset_2` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_offset_2`
