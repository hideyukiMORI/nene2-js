# FT318: notes limit+offset combo 3
...
notes limit+offset combo 3. Handler `bulk_q_notes_combo_3` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT318"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT318 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_3` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_3`
