...
content-type variant 2. Handler `bulk_ct_2` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT481"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT481 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_2` completed under Vitest.
