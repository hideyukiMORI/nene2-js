...
bearer protected. Handler `hdr_bearer` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT73"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT73 | 1 | PASS (automated) |
...
- Scenario `hdr_bearer` completed under Vitest.
- Persona A path: JWT
