# FT324: notes limit+offset combo 6
...
notes limit+offset combo 6. Handler `bulk_q_notes_combo_6` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT324"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT324 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_6` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_6`
