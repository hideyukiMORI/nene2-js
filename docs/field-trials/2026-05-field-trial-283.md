...
tags list limit=150. Handler `bulk_q_tags_limit_150` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT283"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT283 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_150` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_limit_150`
