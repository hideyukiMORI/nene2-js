...
createTag body. Handler `tags_create_body` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT63"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT63 | 1     | PASS (automated) |
...
- Scenario `tags_create_body` completed under Vitest.
- Persona A path: POST JSON
...
### O1: 自動シナリオ `tags_create_body`

カタログ `tags` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
