...
machineHealth 401. Handler `machine_401` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT42"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT42 | 1 | PASS (automated) |
...
- Scenario `machine_401` completed under Vitest.
- Persona A path: Missing api key
