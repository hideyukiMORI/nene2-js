...
X-NENE2-API-Key name. Handler `hdr_apikey_name` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT75"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT75 | 1 | PASS (automated) |
...
- Scenario `hdr_apikey_name` completed under Vitest.
- Persona A path: Spec header
