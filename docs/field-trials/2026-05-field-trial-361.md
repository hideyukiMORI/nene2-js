...
getNote path id=42. Handler `bulk_notes_id_42` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT361"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT361 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_id_42` completed under Vitest.
