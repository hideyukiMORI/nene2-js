...
misc client probe 11. Handler `bulk_misc_11` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT528"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT528 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_11` completed under Vitest.
