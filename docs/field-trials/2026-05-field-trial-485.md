...
content-type variant 6. Handler `bulk_ct_6` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT485"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT485 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_6` completed under Vitest.
