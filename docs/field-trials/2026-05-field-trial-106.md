...
non-client undefined. Handler `vdx_non_client` (fixture, category `validation-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT106"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT106 | 1     | PASS (automated) |
...
- Scenario `vdx_non_client` completed under Vitest.
- Persona A path: Unknown throw
