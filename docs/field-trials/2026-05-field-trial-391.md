# FT391: validation field index 6
...
validation field index 6. Handler `bulk_vdx_fields_6` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT391"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT391 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_6` completed under Vitest.
