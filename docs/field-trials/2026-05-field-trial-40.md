...
smoke parallel. Handler `smoke_ok` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT40"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT40 | 1 | PASS (automated) |
...
- Persona A path: FT12 single call
