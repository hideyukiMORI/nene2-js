# FT422: parseProblemDetails case 12
...
parseProblemDetails case 12. Handler `bulk_prob_parse_12` (fixture, category `problem-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT422"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT422 | 1     | PASS (automated) |
...
- Scenario `bulk_prob_parse_12` completed under Vitest.
