# FT242: problem HTTP 413 on getNote
...
problem HTTP 413 on getNote. Handler `bulk_err_413` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT242"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT242 | 1     | PASS (automated) |
...
- Scenario `bulk_err_413` completed under Vitest.
- Persona A path: Error surface 413
