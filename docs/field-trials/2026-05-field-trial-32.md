...
health 503 allowDegraded. Handler `health_503_allow` (fixture, category `health`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT32"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT32 | 1 | PASS (automated) |
...
- Scenario `health_503_allow` completed under Vitest.
- Persona A path: SRE opt-in degraded
