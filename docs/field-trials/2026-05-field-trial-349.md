# FT349: tags limit+offset combo 18
...
tags limit+offset combo 18. Handler `bulk_q_tags_combo_18` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT349"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT349 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_18` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_combo_18`
