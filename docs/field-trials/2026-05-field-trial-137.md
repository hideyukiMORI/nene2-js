# FT137: [ts_app/python_first] ja doc: getting_started_auth_env
...
[ts_app/python_first] ja doc: getting_started_auth_env. Handler `onb_doc_ts_app_python_first_getting_started_auth_env` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `onb_doc_ts_app_python_first_getting_started_auth_env`                                 |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT137"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT137 | 1     | PASS (automated) |
...
- Scenario `onb_doc_ts_app_python_first_getting_started_auth_env` completed under Vitest.
...
シナリオ `onb_doc_ts_app_python_first_getting_started_auth_env` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
