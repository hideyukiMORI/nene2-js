...
deleteNote id=10000. Handler `bulk_notes_del_10000` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT375"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT375 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_del_10000` completed under Vitest.
...
### O1: 自動シナリオ `bulk_notes_del_10000`
