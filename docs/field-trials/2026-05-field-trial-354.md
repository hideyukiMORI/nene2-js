...
deleteNote id=1. Handler `bulk_notes_del_1` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT354"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT354 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_del_1` completed under Vitest.
