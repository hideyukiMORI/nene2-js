...
parallel calls batch 5. Handler `bulk_conc_5` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT455"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT455 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_5` completed under Vitest.
