# FT245: problem HTTP 422 on createNote
...
problem HTTP 422 on createNote. Handler `bulk_err_422_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT245"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT245 | 1     | PASS (automated) |
...
- Scenario `bulk_err_422_create` completed under Vitest.
- Persona A path: POST failure 422
