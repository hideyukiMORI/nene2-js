...
tags list limit=3. Handler `bulk_q_tags_limit_3` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT261"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT261 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_3` completed under Vitest.
