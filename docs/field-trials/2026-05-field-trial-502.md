...
replay protected_ok #15. Handler `protected_ok` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT502"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT502 | 1     | PASS (automated) |
...
- Scenario `protected_ok` completed under Vitest.
- Persona A path: Regression replay
...
### O1: 自動シナリオ `protected_ok`

カタログ `replay-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
