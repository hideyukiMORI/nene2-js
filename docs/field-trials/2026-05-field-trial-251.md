# FT251: problem HTTP 502 on createNote
...
problem HTTP 502 on createNote. Handler `bulk_err_502_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT251"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT251 | 1     | PASS (automated) |
...
- Scenario `bulk_err_502_create` completed under Vitest.
- Persona A path: POST failure 502
