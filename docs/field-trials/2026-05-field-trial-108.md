...
type URI match. Handler `vdx_type_uri` (fixture, category `validation-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT108"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT108 | 1     | PASS (automated) |
...
- Scenario `vdx_type_uri` completed under Vitest.
- Persona A path: NENE2 type
