# FT231: problem HTTP 400 on createNote
...
problem HTTP 400 on createNote. Handler `bulk_err_400_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT231"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT231 | 1     | PASS (automated) |
...
- Scenario `bulk_err_400_create` completed under Vitest.
- Persona A path: POST failure 400
