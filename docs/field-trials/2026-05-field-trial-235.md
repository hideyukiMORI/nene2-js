# FT235: problem HTTP 403 on createNote
...
problem HTTP 403 on createNote. Handler `bulk_err_403_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT235"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT235 | 1     | PASS (automated) |
...
- Scenario `bulk_err_403_create` completed under Vitest.
- Persona A path: POST failure 403
