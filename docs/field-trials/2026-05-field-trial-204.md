# FT204: [ts_app] client: notes_list_fixture
...
[ts_app] client: notes_list_fixture. Handler `onb_client_ts_app_notes_list_fixture` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_client_ts_app_notes_list_fixture` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT204"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT204 | 1 | PASS (automated) |
...
- Scenario `onb_client_ts_app_notes_list_fixture` completed under Vitest.
...
### O1: 自動シナリオ `onb_client_ts_app_notes_list_fixture`
