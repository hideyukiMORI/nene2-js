...
getNote path id=2. Handler `bulk_notes_id_2` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT355"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT355 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_id_2` completed under Vitest.
