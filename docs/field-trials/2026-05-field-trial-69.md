...
invalid jwt. Handler `protected_bad_token` (fixture, category `protected`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT69"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT69 | 1 | PASS (automated) |
...
- Scenario `protected_bad_token` completed under Vitest.
- Persona A path: Expired token
