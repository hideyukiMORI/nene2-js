...
content-type variant 7. Handler `bulk_ct_7` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT486"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT486 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_7` completed under Vitest.
- Persona A path: MIME parsing
...
カタログ `content-type-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
