...
live onboarding 3. Handler `onb_live_matrix_skip_unset` (live, category `docs-live-onboarding`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
| Handler | `onb_live_matrix_skip_unset` |
| Mode | live |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT227"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT227 | 1 | PASS (automated) |
...
- Scenario `onb_live_matrix_skip_unset` completed under Vitest.
- Persona A path: サーバ起動後に ja howto/live-smoke を実行
- Persona B path: フロント/スクリプトから TS クライアントを足す
...
### O1: 自動シナリオ `onb_live_matrix_skip_unset`
