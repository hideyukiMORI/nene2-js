...
getNote. Handler `notes_get` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT47"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT47 | 1 | PASS (automated) |
...
- Scenario `notes_get` completed under Vitest.
- Persona A path: Detail view
