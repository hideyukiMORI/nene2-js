...
fromClientError. Handler `vdx_from_err` (fixture, category `validation-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT104"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT104 | 1 | PASS (automated) |
...
- Scenario `vdx_from_err` completed under Vitest.
- Persona A path: Catch UX
