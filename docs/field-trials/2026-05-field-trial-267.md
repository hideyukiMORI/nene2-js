...
tags list limit=10. Handler `bulk_q_tags_limit_10` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT267"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT267 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_10` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_limit_10`
