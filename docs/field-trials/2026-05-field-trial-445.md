...
auth header combo 10. Handler `bulk_hdr_variant_10` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT445"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT445 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_10` completed under Vitest.
