...
live onboarding 2. Handler `onb_live_smoke_after_python` (live, category `docs-live-onboarding`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
| Handler | `onb_live_smoke_after_python` |
| Mode | live |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT226"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT226 | 1 | PASS (automated) |
...
- Scenario `onb_live_smoke_after_python` completed under Vitest.
- Persona A path: サーバ起動後に ja howto/live-smoke を実行
- Persona B path: PHP NENE2 は未経験、parity で JS クライアントを試す
...
### O1: 自動シナリオ `onb_live_smoke_after_python`
