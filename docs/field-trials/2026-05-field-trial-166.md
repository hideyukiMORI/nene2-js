# FT166: [php_dev/python_first] ja doc: getting_started_npm
...
[php_dev/python_first] ja doc: getting_started_npm. Handler `onb_doc_php_dev_python_first_getting_started_npm` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_php_dev_python_first_getting_started_npm` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT166"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT166 | 1 | PASS (automated) |
...
- Scenario `onb_doc_php_dev_python_first_getting_started_npm` completed under Vitest.
...
シナリオ `onb_doc_php_dev_python_first_getting_started_npm` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
