...
replay notes_list #23. Handler `notes_list` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT510"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT510 | 1     | PASS (automated) |
...
- Scenario `notes_list` completed under Vitest.
