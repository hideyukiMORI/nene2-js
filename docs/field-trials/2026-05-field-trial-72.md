...
apiKey machine. Handler `hdr_machine_key` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT72"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT72 | 1 | PASS (automated) |
...
- Scenario `hdr_machine_key` completed under Vitest.
- Persona A path: Ops key
