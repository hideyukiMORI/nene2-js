...
updateNote id=999. Handler `bulk_notes_upd_999` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT371"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT371 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_upd_999` completed under Vitest.
