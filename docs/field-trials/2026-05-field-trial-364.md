# FT364: generated schema probe 2

**Date**: 2026-05-22  
**Issues / PRs**: [#42](https://github.com/hideyukiMORI/nene2-js/issues/42) (marathon 500)

---

## Theme

generated schema probe 2 (`bulk_schema_2`, fixture).

## What was exercised

`npm run test:ft-marathon` — category `openapi-bulk`.

## DX Review

### Persona A — TypeScript app developer

OpenAPI type alignment **Risk:** low

### Persona B — New to NENE2

Persona B: marathon bulk regression. **Risk:** low

## Friction points

No actionable friction — automated scenario passed.

## Follow-up Issues

| Repo                  | Issue | Status |
| --------------------- | ----- | ------ |
| hideyukiMORI/nene2-js | #42   | merged |

**FT completion:** done
