...
misc client probe 7. Handler `bulk_misc_7` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT524"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT524 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_7` completed under Vitest.
