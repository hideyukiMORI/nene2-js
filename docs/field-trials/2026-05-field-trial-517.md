...
misc client probe 0. Handler `bulk_misc_0` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT517"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT517 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_0` completed under Vitest.
