...
getNote path id=999. Handler `bulk_notes_id_999` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT370"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT370 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_id_999` completed under Vitest.
