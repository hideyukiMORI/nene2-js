...
tags limit. Handler `q_tags_limit` (fixture, category `query`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT117"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT117 | 1 | PASS (automated) |
...
- Scenario `q_tags_limit` completed under Vitest.
