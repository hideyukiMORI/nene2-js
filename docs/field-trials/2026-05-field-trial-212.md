# FT212: [py_dev] client: smoke_fixture
...
[py_dev] client: smoke_fixture. Handler `onb_client_py_dev_smoke_fixture` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_client_py_dev_smoke_fixture` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT212"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT212 | 1 | PASS (automated) |
...
- Scenario `onb_client_py_dev_smoke_fixture` completed under Vitest.
...
### O1: 自動シナリオ `onb_client_py_dev_smoke_fixture`
