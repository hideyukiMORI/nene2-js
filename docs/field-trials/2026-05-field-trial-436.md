...
auth header combo 1. Handler `bulk_hdr_variant_1` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT436"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT436 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_1` completed under Vitest.
