# FT330: notes limit+offset combo 9
...
notes limit+offset combo 9. Handler `bulk_q_notes_combo_9` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT330"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT330 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_9` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_9`
