...
html 200. Handler `err_html` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT93"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT93 | 1     | PASS (automated) |
...
- Scenario `err_html` completed under Vitest.
- Persona A path: Wrong port
