...
getProtected 401. Handler `protected_401` (fixture, category `protected`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT67"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT67 | 1 | PASS (automated) |
...
- Scenario `protected_401` completed under Vitest.
- Persona A path: Login redirect
