# FT200: [ai_agent/python_first] ja doc: getting_started_curl_health
...
[ai_agent/python_first] ja doc: getting_started_curl_health. Handler `onb_doc_ai_agent_python_first_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_ai_agent_python_first_getting_started_curl_health` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT200"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT200 | 1 | PASS (automated) |
...
- Scenario `onb_doc_ai_agent_python_first_getting_started_curl_health` completed under Vitest.
- Persona A path: AI エージェント（ドキュメント URL のみ入力）
- Persona B path: AGENTS.md なしで ja サイトだけ渡された想定 / nene2-python起動→NENE2確認→npm client
...
シナリオ `onb_doc_ai_agent_python_first_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
