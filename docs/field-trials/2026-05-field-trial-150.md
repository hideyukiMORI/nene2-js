# FT150: [py_dev/python_first] ja doc: getting_started_npm
...
[py_dev/python_first] ja doc: getting_started_npm. Handler `onb_doc_py_dev_python_first_getting_started_npm` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_py_dev_python_first_getting_started_npm` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT150"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT150 | 1 | PASS (automated) |
...
- Scenario `onb_doc_py_dev_python_first_getting_started_npm` completed under Vitest.
...
シナリオ `onb_doc_py_dev_python_first_getting_started_npm` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
