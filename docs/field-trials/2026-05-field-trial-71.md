...
welcome message. Handler `protected_message` (fixture, category `protected`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT71"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT71 | 1 | PASS (automated) |
...
- Scenario `protected_message` completed under Vitest.
- Persona A path: Copy display
...
### O1: 自動シナリオ `protected_message`

カタログ `protected` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
