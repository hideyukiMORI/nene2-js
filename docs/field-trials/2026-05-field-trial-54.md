...
update path. Handler `notes_update_path` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT54"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT54 | 1 | PASS (automated) |
...
- Scenario `notes_update_path` completed under Vitest.
- Persona A path: URL id
