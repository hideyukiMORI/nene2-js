...
custom fetch. Handler `cfg_custom_fetch` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT81"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT81 | 1 | PASS (automated) |
...
- Scenario `cfg_custom_fetch` completed under Vitest.
- Persona A path: Test double
