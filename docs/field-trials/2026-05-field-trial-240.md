# FT240: problem HTTP 409 on getNote
...
problem HTTP 409 on getNote. Handler `bulk_err_409` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT240"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT240 | 1     | PASS (automated) |
...
- Scenario `bulk_err_409` completed under Vitest.
- Persona A path: Error surface 409
