...
bearer header. Handler `protected_bearer_hdr` (fixture, category `protected`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT68"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT68 | 1 | PASS (automated) |
...
- Scenario `protected_bearer_hdr` completed under Vitest.
- Persona A path: Header inspect
...
### O1: 自動シナリオ `protected_bearer_hdr`
