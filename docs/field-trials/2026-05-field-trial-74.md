...
both headers. Handler `hdr_both` (fixture, category `auth-headers`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT74"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT74 | 1 | PASS (automated) |
...
- Scenario `hdr_both` completed under Vitest.
- Persona A path: Mixed clients
