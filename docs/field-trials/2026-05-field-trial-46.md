...
listNotes query. Handler `notes_list_query` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT46"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT46 | 1     | PASS (automated) |
...
- Scenario `notes_list_query` completed under Vitest.
- Persona A path: Pagination
