...
baseUrl variant 0. Handler `bulk_cfg_base_0` (fixture, category `config-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT430"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT430 | 1     | PASS (automated) |
...
- Scenario `bulk_cfg_base_0` completed under Vitest.
