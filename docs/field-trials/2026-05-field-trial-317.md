# FT317: tags limit+offset combo 2
...
tags limit+offset combo 2. Handler `bulk_q_tags_combo_2` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT317"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT317 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_2` completed under Vitest.
