...
createNote 422. Handler `notes_create_422` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT52"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT52 | 1     | PASS (automated) |
...
- Scenario `notes_create_422` completed under Vitest.
- Persona A path: Form validation map
