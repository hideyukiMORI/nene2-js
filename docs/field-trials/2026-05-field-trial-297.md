...
tags offset=5. Handler `bulk_q_tags_offset_5` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT297"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT297 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_offset_5` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_offset_5`
