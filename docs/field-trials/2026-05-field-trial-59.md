...
createTag. Handler `tags_create` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT59"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT59 | 1 | PASS (automated) |
...
- Scenario `tags_create` completed under Vitest.
- Persona A path: Add tag
