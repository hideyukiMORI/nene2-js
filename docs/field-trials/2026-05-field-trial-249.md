# FT249: problem HTTP 500 on createNote
...
problem HTTP 500 on createNote. Handler `bulk_err_500_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT249"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT249 | 1     | PASS (automated) |
...
- Scenario `bulk_err_500_create` completed under Vitest.
- Persona A path: POST failure 500
