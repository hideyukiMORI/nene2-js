...
auth header combo 7. Handler `bulk_hdr_variant_7` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT442"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT442 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_7` completed under Vitest.
