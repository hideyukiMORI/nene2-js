# FT169: [php_dev/python_first] ja doc: getting_started_auth_env
...
[php_dev/python_first] ja doc: getting_started_auth_env. Handler `onb_doc_php_dev_python_first_getting_started_auth_env` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `onb_doc_php_dev_python_first_getting_started_auth_env`                                |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT169"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT169 | 1     | PASS (automated) |
...
- Scenario `onb_doc_php_dev_python_first_getting_started_auth_env` completed under Vitest.
...
シナリオ `onb_doc_php_dev_python_first_getting_started_auth_env` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
