# FT334: notes limit+offset combo 11
...
notes limit+offset combo 11. Handler `bulk_q_notes_combo_11` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT334"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT334 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_11` completed under Vitest.
- Persona A path: Combined query params
...
### O1: 自動シナリオ `bulk_q_notes_combo_11`

カタログ `query-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
