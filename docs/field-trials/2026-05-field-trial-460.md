...
parallel calls batch 10. Handler `bulk_conc_10` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT460"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT460 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_10` completed under Vitest.
