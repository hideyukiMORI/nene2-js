...
auth header combo 6. Handler `bulk_hdr_variant_6` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT441"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT441 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_6` completed under Vitest.
