...
notes list limit=5. Handler `bulk_q_notes_limit_5` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT262"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT262 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_limit_5` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_limit_5`
