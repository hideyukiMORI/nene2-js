# FT411: parseProblemDetails case 1
...
parseProblemDetails case 1. Handler `bulk_prob_parse_1` (fixture, category `problem-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT411"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT411 | 1     | PASS (automated) |
...
- Scenario `bulk_prob_parse_1` completed under Vitest.
