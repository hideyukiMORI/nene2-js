...
non-problem undefined. Handler `prob_non` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT103"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT103 | 1 | PASS (automated) |
...
- Scenario `prob_non` completed under Vitest.
- Persona A path: Fallback
