...
replay health_ok #20. Handler `health_ok` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT507"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT507 | 1     | PASS (automated) |
...
- Scenario `health_ok` completed under Vitest.
