...
updateNote id=100. Handler `bulk_notes_upd_100` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT368"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT368 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_upd_100` completed under Vitest.
