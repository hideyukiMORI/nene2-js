...
empty baseUrl. Handler `cfg_empty_url` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT80"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT80 | 1 | PASS (automated) |
...
- Scenario `cfg_empty_url` completed under Vitest.
- Persona A path: Fail fast
