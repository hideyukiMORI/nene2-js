...
live notes CRUD. Handler `live_notes_crud` (live, category `live-evac`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT125"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT125 | 1 | PASS (automated) |
...
- Scenario `live_notes_crud` completed under Vitest.
- Persona A path: Persona A: live evac 18080.
...
### O1: 自動シナリオ `live_notes_crud`

カタログ `live-evac` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
