# FT319: tags limit+offset combo 3
...
tags limit+offset combo 3. Handler `bulk_q_tags_combo_3` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT319"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT319 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_3` completed under Vitest.
