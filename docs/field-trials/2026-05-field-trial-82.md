...
AbortSignal. Handler `cfg_signal` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT82"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT82 | 1     | PASS (automated) |
...
- Scenario `cfg_signal` completed under Vitest.
- Persona A path: Cancel refresh
