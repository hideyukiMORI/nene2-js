...
tags list limit=999. Handler `bulk_q_tags_limit_999` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT289"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT289 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_999` completed under Vitest.
- Persona A path: Tags pagination
...
### O1: 自動シナリオ `bulk_q_tags_limit_999`
