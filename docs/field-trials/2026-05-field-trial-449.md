...
auth header combo 14. Handler `bulk_hdr_variant_14` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT449"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT449 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_14` completed under Vitest.
- Persona A path: Header matrix
...
### O1: 自動シナリオ `bulk_hdr_variant_14`

カタログ `auth-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
