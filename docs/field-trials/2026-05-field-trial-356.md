...
updateNote id=2. Handler `bulk_notes_upd_2` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT356"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT356 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_upd_2` completed under Vitest.
