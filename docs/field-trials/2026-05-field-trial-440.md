...
auth header combo 5. Handler `bulk_hdr_variant_5` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT440"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT440 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_5` completed under Vitest.
