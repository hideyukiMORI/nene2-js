...
auth header combo 12. Handler `bulk_hdr_variant_12` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT447"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT447 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_12` completed under Vitest.
