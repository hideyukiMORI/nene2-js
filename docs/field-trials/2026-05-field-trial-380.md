...
getTag id=3. Handler `bulk_tags_id_3` (fixture, category `tags-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT380"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT380 | 1     | PASS (automated) |
...
- Scenario `bulk_tags_id_3` completed under Vitest.
