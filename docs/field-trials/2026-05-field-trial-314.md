# FT314: notes limit+offset combo 1
...
notes limit+offset combo 1. Handler `bulk_q_notes_combo_1` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT314"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT314 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_1` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_1`
