...
content-type variant 3. Handler `bulk_ct_3` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT482"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT482 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_3` completed under Vitest.
