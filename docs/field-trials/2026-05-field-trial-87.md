...
404 not-found. Handler `err_404` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT87"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT87 | 1     | PASS (automated) |
...
- Scenario `err_404` completed under Vitest.
- Persona A path: Missing resource
