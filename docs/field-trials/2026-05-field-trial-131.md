# FT131: [ts_app/nene2_first] ja doc: getting_started_smoke_code
...
[ts_app/nene2_first] ja doc: getting_started_smoke_code. Handler `onb_doc_ts_app_nene2_first_getting_started_smoke_code` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_ts_app_nene2_first_getting_started_smoke_code` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT131"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT131 | 1 | PASS (automated) |
...
- Scenario `onb_doc_ts_app_nene2_first_getting_started_smoke_code` completed under Vitest.
...
シナリオ `onb_doc_ts_app_nene2_first_getting_started_smoke_code` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
