# FT209: [ts_app] client: abort_signal
...
[ts_app] client: abort_signal. Handler `onb_client_ts_app_abort_signal` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_client_ts_app_abort_signal` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT209"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT209 | 1 | PASS (automated) |
...
- Scenario `onb_client_ts_app_abort_signal` completed under Vitest.
...
### O1: 自動シナリオ `onb_client_ts_app_abort_signal`
