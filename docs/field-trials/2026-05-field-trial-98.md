...
isValidationPD. Handler `prob_is_vpd` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT98"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT98 | 1     | PASS (automated) |
...
- Scenario `prob_is_vpd` completed under Vitest.
- Persona A path: Form map
