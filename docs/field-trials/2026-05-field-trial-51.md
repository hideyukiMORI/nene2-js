...
getNote 404. Handler `notes_get_404` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT51"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT51 | 1     | PASS (automated) |
...
- Scenario `notes_get_404` completed under Vitest.
- Persona A path: Missing id UX
