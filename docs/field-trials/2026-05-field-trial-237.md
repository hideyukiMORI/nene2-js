# FT237: problem HTTP 404 on createNote
...
problem HTTP 404 on createNote. Handler `bulk_err_404_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT237"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT237 | 1     | PASS (automated) |
...
- Scenario `bulk_err_404_create` completed under Vitest.
- Persona A path: POST failure 404
