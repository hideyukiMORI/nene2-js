...
baseUrl variant 1. Handler `bulk_cfg_base_1` (fixture, category `config-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT431"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT431 | 1     | PASS (automated) |
...
- Scenario `bulk_cfg_base_1` completed under Vitest.
