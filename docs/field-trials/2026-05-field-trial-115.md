...
notes offset. Handler `q_notes_offset` (fixture, category `query`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT115"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT115 | 1     | PASS (automated) |
...
- Scenario `q_notes_offset` completed under Vitest.
- Persona A path: Page 2
