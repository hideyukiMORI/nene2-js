# FT410: parseProblemDetails case 0
...
parseProblemDetails case 0. Handler `bulk_prob_parse_0` (fixture, category `problem-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT410"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT410 | 1     | PASS (automated) |
...
- Scenario `bulk_prob_parse_0` completed under Vitest.
