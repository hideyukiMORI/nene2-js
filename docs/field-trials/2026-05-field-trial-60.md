...
updateTag. Handler `tags_update` (fixture, category `tags`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT60"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT60 | 1 | PASS (automated) |
...
- Scenario `tags_update` completed under Vitest.
- Persona A path: Rename
