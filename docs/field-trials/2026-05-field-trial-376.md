# FT376: getNote path id=2147483646
...
getNote path id=2147483646. Handler `bulk_notes_id_2147483646` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| ----------- | -------------------------------------------------------------------------------------- |
| Category    | `notes-bulk`                                                                           |
| Handler     | `bulk_notes_id_2147483646`                                                             |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT376"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT376 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_id_2147483646` completed under Vitest.
- Persona A path: Path param integer
...
### O1: 自動シナリオ `bulk_notes_id_2147483646`

カタログ `notes-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
