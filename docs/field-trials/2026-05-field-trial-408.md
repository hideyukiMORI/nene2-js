# FT408: validation field index 23
...
validation field index 23. Handler `bulk_vdx_fields_23` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT408"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT408 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_23` completed under Vitest.
- Persona A path: 422 field mapping
...
### O1: 自動シナリオ `bulk_vdx_fields_23`

カタログ `validation-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
