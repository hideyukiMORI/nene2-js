...
live health. Handler `live_health` (live, category `live-evac`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT119"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT119 | 1 | PASS (automated) |
...
- Scenario `live_health` completed under Vitest.
