...
isValidationError. Handler `prob_is_ve` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT102"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT102 | 1     | PASS (automated) |
...
- Scenario `prob_is_ve` completed under Vitest.
- Persona A path: Field row
