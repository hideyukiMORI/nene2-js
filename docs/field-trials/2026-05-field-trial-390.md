# FT390: validation field index 5
...
validation field index 5. Handler `bulk_vdx_fields_5` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT390"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT390 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_5` completed under Vitest.
