# FT338: notes limit+offset combo 13
...
notes limit+offset combo 13. Handler `bulk_q_notes_combo_13` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT338"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT338 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_13` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_13`
