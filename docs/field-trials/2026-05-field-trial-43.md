...
framework bad status. Handler `framework_bad` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT43"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT43 | 1 | PASS (automated) |
...
- Scenario `framework_bad` completed under Vitest.
- Persona A path: Strict guard
...
### O1: 自動シナリオ `framework_bad`

カタログ `system` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
