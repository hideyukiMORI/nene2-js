...
extensions. Handler `prob_extensions` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT101"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT101 | 1 | PASS (automated) |
...
- Scenario `prob_extensions` completed under Vitest.
- Persona A path: Extra fields
