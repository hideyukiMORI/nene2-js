...
tags list limit=100. Handler `bulk_q_tags_limit_100` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT281"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT281 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_100` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_limit_100`
