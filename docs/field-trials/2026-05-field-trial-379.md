...
getTag id=1. Handler `bulk_tags_id_1` (fixture, category `tags-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT379"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT379 | 1     | PASS (automated) |
...
- Scenario `bulk_tags_id_1` completed under Vitest.
