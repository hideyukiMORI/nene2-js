...
[py_dev] client: machine_health_401. Handler `onb_client_py_dev_machine_health_401` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `onb_client_py_dev_machine_health_401`                                                 |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT220"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT220 | 1     | PASS (automated) |
...
- Scenario `onb_client_py_dev_machine_health_401` completed under Vitest.
...
### O1: 自動シナリオ `onb_client_py_dev_machine_health_401`
