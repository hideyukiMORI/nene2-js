...
notes limit. Handler `q_notes_limit` (fixture, category `query`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT114"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT114 | 1     | PASS (automated) |
...
- Scenario `q_notes_limit` completed under Vitest.
- Persona A path: Page size
