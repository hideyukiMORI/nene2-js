...
getTag id=200. Handler `bulk_tags_id_200` (fixture, category `tags-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT383"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT383 | 1     | PASS (automated) |
...
- Scenario `bulk_tags_id_200` completed under Vitest.
