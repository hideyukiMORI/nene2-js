...
health html wrong port. Handler `health_html` (fixture, category `health`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT36"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT36 | 1     | PASS (automated) |
...
- Scenario `health_html` completed under Vitest.
- Persona A path: FT15 html hint
