# FT394: validation field index 9
...
validation field index 9. Handler `bulk_vdx_fields_9` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT394"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT394 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_9` completed under Vitest.
