# FT466: generated schema probe 4
...
generated schema probe 4. Handler `bulk_schema_4` (fixture, category `openapi-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT466"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT466 | 1     | PASS (automated) |
...
- Scenario `bulk_schema_4` completed under Vitest.
- Persona A path: OpenAPI type alignment
...
### O1: 自動シナリオ `bulk_schema_4`

カタログ `openapi-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
