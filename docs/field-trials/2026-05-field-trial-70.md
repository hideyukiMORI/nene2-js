...
claims sub. Handler `protected_claims` (fixture, category `protected`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT70"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT70 | 1 | PASS (automated) |
...
- Scenario `protected_claims` completed under Vitest.
- Persona A path: User id
