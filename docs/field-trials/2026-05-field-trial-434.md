...
baseUrl variant 4. Handler `bulk_cfg_base_4` (fixture, category `config-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT434"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT434 | 1     | PASS (automated) |
...
- Scenario `bulk_cfg_base_4` completed under Vitest.
- Persona A path: URL normalization
...
### O1: 自動シナリオ `bulk_cfg_base_4`

カタログ `config-bulk` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
