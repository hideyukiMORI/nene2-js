...
parallel lists. Handler `conc_lists` (fixture, category `concurrency`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT111"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT111 | 1 | PASS (automated) |
...
- Scenario `conc_lists` completed under Vitest.
- Persona A path: Dashboard
