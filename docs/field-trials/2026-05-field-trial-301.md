...
tags offset=20. Handler `bulk_q_tags_offset_20` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT301"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT301 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_offset_20` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_offset_20`
