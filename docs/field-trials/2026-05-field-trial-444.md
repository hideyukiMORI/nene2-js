...
auth header combo 9. Handler `bulk_hdr_variant_9` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT444"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT444 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_9` completed under Vitest.
