...
misc client probe 1. Handler `bulk_misc_1` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT518"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT518 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_1` completed under Vitest.
