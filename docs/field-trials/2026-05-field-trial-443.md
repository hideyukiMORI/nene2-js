...
auth header combo 8. Handler `bulk_hdr_variant_8` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT443"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT443 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_8` completed under Vitest.
