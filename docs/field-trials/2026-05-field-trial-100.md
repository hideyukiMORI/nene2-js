...
validation type URI. Handler `prob_val_type` (fixture, category `problem`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT100"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT100 | 1 | PASS (automated) |
...
- Scenario `prob_val_type` completed under Vitest.
- Persona A path: Branch UI
...
### O1: 自動シナリオ `prob_val_type`

カタログ `problem` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
