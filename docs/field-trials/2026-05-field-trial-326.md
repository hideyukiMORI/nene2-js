# FT326: notes limit+offset combo 7
...
notes limit+offset combo 7. Handler `bulk_q_notes_combo_7` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT326"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT326 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_7` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_7`
