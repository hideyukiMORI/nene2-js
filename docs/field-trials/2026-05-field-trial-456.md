...
parallel calls batch 6. Handler `bulk_conc_6` (fixture, category `concurrency-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT456"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT456 | 1     | PASS (automated) |
...
- Scenario `bulk_conc_6` completed under Vitest.
