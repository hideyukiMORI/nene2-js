...
auth header combo 2. Handler `bulk_hdr_variant_2` (fixture, category `auth-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT437"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT437 | 1     | PASS (automated) |
...
- Scenario `bulk_hdr_variant_2` completed under Vitest.
