...
tags list limit=2. Handler `bulk_q_tags_limit_2` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT259"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT259 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_2` completed under Vitest.
