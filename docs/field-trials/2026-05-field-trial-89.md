...
500 internal. Handler `err_500` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT89"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT89 | 1 | PASS (automated) |
...
- Scenario `err_500` completed under Vitest.
- Persona A path: Retry UX
