# FT218: [py_dev] client: baseurl_trailing_slash
...
[py_dev] client: baseurl_trailing_slash. Handler `onb_client_py_dev_baseurl_trailing_slash` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler     | `onb_client_py_dev_baseurl_trailing_slash`                                             |
| Mode        | fixture                                                                                |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT218"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT218 | 1     | PASS (automated) |
...
- Scenario `onb_client_py_dev_baseurl_trailing_slash` completed under Vitest.
- Persona A path: Python 開発者（nene2-python のみ知っている）
- Persona B path: ja チュートリアルのコードをそのまま実行
...
### O1: 自動シナリオ `onb_client_py_dev_baseurl_trailing_slash`

カタログ `docs-client-onboarding` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
