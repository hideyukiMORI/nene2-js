...
tags list limit=25. Handler `bulk_q_tags_limit_25` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT273"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT273 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_25` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_limit_25`
