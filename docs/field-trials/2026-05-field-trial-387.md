# FT387: validation field index 2
...
validation field index 2. Handler `bulk_vdx_fields_2` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT387"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT387 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_2` completed under Vitest.
