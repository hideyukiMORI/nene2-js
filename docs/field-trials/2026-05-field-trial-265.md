...
tags list limit=7. Handler `bulk_q_tags_limit_7` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT265"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT265 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_7` completed under Vitest.
