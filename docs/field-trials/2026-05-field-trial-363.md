...
deleteNote id=42. Handler `bulk_notes_del_42` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT363"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT363 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_del_42` completed under Vitest.
