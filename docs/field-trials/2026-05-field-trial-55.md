...
delete path. Handler `notes_delete_path` (fixture, category `notes`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT55"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT55 | 1     | PASS (automated) |
...
- Scenario `notes_delete_path` completed under Vitest.
