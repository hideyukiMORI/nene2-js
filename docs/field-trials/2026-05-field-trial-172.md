# FT172: [php_dev/alt_ports] ja doc: getting_started_curl_health
...
[php_dev/alt_ports] ja doc: getting_started_curl_health. Handler `onb_doc_php_dev_alt_ports_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_php_dev_alt_ports_getting_started_curl_health` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT172"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT172 | 1 | PASS (automated) |
...
- Scenario `onb_doc_php_dev_alt_ports_getting_started_curl_health` completed under Vitest.
- Persona A path: NENE2 PHP 開発者（Composer 慣れ）
- Persona B path: フロント/スクリプトから TS クライアントを足す / evac :18080 / python :8000 の切替
...
シナリオ `onb_doc_php_dev_alt_ports_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
