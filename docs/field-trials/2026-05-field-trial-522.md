...
misc client probe 5. Handler `bulk_misc_5` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT522"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT522 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_5` completed under Vitest.
