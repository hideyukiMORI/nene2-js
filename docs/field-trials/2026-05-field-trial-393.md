# FT393: validation field index 8
...
validation field index 8. Handler `bulk_vdx_fields_8` (fixture, category `validation-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT393"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT393 | 1     | PASS (automated) |
...
- Scenario `bulk_vdx_fields_8` completed under Vitest.
