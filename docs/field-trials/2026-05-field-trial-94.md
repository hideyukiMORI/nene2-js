...
204 delete. Handler `err_204` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT94"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT94 | 1 | PASS (automated) |
...
- Scenario `err_204` completed under Vitest.
- Persona A path: No body
