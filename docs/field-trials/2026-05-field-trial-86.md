...
strip only trailing. Handler `cfg_strip_trail` (fixture, category `config-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT86"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT86 | 1     | PASS (automated) |
...
- Scenario `cfg_strip_trail` completed under Vitest.
- Persona A path: Path preserve
...
### O1: 自動シナリオ `cfg_strip_trail`

カタログ `config-dx` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
