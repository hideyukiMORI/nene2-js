# FT248: problem HTTP 500 on getNote
...
problem HTTP 500 on getNote. Handler `bulk_err_500` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT248"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT248 | 1     | PASS (automated) |
...
- Scenario `bulk_err_500` completed under Vitest.
- Persona A path: Error surface 500
