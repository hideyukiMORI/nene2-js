...
error code field. Handler `vdx_code` (fixture, category `validation-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT109"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT109 | 1 | PASS (automated) |
...
- Scenario `vdx_code` completed under Vitest.
- Persona A path: i18n key
