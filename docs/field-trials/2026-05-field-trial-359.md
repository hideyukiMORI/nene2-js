...
updateNote id=7. Handler `bulk_notes_upd_7` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT359"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT359 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_upd_7` completed under Vitest.
