# FT180: [devops/nene2_first] ja doc: getting_started_curl_health
...
[devops/nene2_first] ja doc: getting_started_curl_health. Handler `onb_doc_devops_nene2_first_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_devops_nene2_first_getting_started_curl_health` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT180"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT180 | 1 | PASS (automated) |
...
- Scenario `onb_doc_devops_nene2_first_getting_started_curl_health` completed under Vitest.
- Persona A path: DevOps / SRE（ヘルスとポートの切り分け）
- Persona B path: 8080/8000/18080 の取り違えを疑う / NENE2(PHP)起動→ja docs→npm client
...
シナリオ `onb_doc_devops_nene2_first_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
