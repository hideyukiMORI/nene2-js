...
live onboarding 1. Handler `onb_live_smoke_after_nene2` (live, category `docs-live-onboarding`). Live — requires `NENE2_JS_API_BASE_URL` and running NENE2 / nene2-python (see ft-evac-ports).
...
| Handler | `onb_live_smoke_after_nene2` |
| Mode | live |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT225"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT225 | 1 | PASS (automated) |
...
- Scenario `onb_live_smoke_after_nene2` completed under Vitest.
- Persona A path: サーバ起動後に ja howto/live-smoke を実行
- Persona B path: hideyukimori.github.io/nene2-js/ja を初めて読む
...
### O1: 自動シナリオ `onb_live_smoke_after_nene2`

カタログ `docs-live-onboarding` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
