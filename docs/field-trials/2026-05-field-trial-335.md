# FT335: tags limit+offset combo 11
...
tags limit+offset combo 11. Handler `bulk_q_tags_combo_11` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT335"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT335 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_11` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_combo_11`
