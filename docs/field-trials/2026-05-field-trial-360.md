...
deleteNote id=7. Handler `bulk_notes_del_7` (fixture, category `notes-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT360"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT360 | 1     | PASS (automated) |
...
- Scenario `bulk_notes_del_7` completed under Vitest.
