...
misc client probe 6. Handler `bulk_misc_6` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT523"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT523 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_6` completed under Vitest.
