...
replay protected_ok #5. Handler `protected_ok` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT492"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT492 | 1     | PASS (automated) |
