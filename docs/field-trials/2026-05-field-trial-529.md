...
misc client probe 12. Handler `bulk_misc_12` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT529"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT529 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_12` completed under Vitest.
