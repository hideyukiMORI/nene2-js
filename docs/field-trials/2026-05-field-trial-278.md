...
notes list limit=99. Handler `bulk_q_notes_limit_99` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT278"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT278 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_limit_99` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_limit_99`
