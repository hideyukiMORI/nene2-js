# FT425: parseProblemDetails case 15
...
parseProblemDetails case 15. Handler `bulk_prob_parse_15` (fixture, category `problem-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT425"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT425 | 1     | PASS (automated) |
...
- Scenario `bulk_prob_parse_15` completed under Vitest.
