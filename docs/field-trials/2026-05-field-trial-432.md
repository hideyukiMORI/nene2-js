...
baseUrl variant 2. Handler `bulk_cfg_base_2` (fixture, category `config-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT432"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT432 | 1     | PASS (automated) |
...
- Scenario `bulk_cfg_base_2` completed under Vitest.
