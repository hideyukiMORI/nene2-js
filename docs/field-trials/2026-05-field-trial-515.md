...
replay vdx_by_field #28. Handler `vdx_by_field` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT515"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT515 | 1     | PASS (automated) |
...
- Scenario `vdx_by_field` completed under Vitest.
