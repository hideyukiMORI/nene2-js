...
replay tags_list #24. Handler `tags_list` (fixture, category `replay-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT511"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT511 | 1     | PASS (automated) |
...
- Scenario `tags_list` completed under Vitest.
