...
tags list limit=0. Handler `bulk_q_tags_limit_0` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT255"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT255 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_limit_0` completed under Vitest.
