...
machineHealth 200. Handler `machine_ok` (fixture, category `system`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT41"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT41 | 1     | PASS (automated) |
...
- Persona A path: Machine monitor
