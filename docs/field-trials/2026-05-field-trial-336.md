# FT336: notes limit+offset combo 12
...
notes limit+offset combo 12. Handler `bulk_q_notes_combo_12` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT336"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT336 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_12` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_12`
