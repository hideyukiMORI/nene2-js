# FT328: notes limit+offset combo 8
...
notes limit+offset combo 8. Handler `bulk_q_notes_combo_8` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT328"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT328 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_combo_8` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_combo_8`
