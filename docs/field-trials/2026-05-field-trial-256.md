...
notes list limit=1. Handler `bulk_q_notes_limit_1` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT256"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT256 | 1     | PASS (automated) |
...
- Scenario `bulk_q_notes_limit_1` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_notes_limit_1`
