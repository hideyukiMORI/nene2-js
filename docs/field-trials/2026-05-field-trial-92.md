...
invalid json. Handler `err_invalid_json` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT92"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT92 | 1     | PASS (automated) |
...
- Scenario `err_invalid_json` completed under Vitest.
- Persona A path: Broken proxy
