...
413 too large. Handler `err_413` (fixture, category `errors`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT91"
...
| Suite              | Tests | Result           |
| ------------------ | ----- | ---------------- |
| `ft-marathon` FT91 | 1     | PASS (automated) |
...
- Scenario `err_413` completed under Vitest.
- Persona A path: Upload limit
