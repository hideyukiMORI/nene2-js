# FT337: tags limit+offset combo 12
...
tags limit+offset combo 12. Handler `bulk_q_tags_combo_12` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT337"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT337 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_12` completed under Vitest.
- Persona A path: Tags combo query
...
### O1: 自動シナリオ `bulk_q_tags_combo_12`
