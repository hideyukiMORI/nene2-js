# FT148: [py_dev/nene2_first] ja doc: getting_started_curl_health
...
[py_dev/nene2_first] ja doc: getting_started_curl_health. Handler `onb_doc_py_dev_nene2_first_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_py_dev_nene2_first_getting_started_curl_health` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT148"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT148 | 1 | PASS (automated) |
...
- Scenario `onb_doc_py_dev_nene2_first_getting_started_curl_health` completed under Vitest.
- Persona A path: Python 開発者（nene2-python のみ知っている）
- Persona B path: PHP NENE2 は未経験、parity で JS クライアントを試す / NENE2(PHP)起動→ja docs→npm client
...
シナリオ `onb_doc_py_dev_nene2_first_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
