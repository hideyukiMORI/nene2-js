...
misc client probe 3. Handler `bulk_misc_3` (fixture, category `misc-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT520"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT520 | 1     | PASS (automated) |
...
- Scenario `bulk_misc_3` completed under Vitest.
