# FT144: [ts_app/client_only] ja doc: getting_started_curl_health
...
[ts_app/client_only] ja doc: getting_started_curl_health. Handler `onb_doc_ts_app_client_only_getting_started_curl_health` (fixture, category `docs-ja-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_doc_ts_app_client_only_getting_started_curl_health` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT144"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT144 | 1 | PASS (automated) |
...
- Scenario `onb_doc_ts_app_client_only_getting_started_curl_health` completed under Vitest.
- Persona A path: TypeScript アプリ開発者（知識ゼロ）
- Persona B path: hideyukimori.github.io/nene2-js/ja を初めて読む / サーバ未起動で client のみ導入
...
シナリオ `onb_doc_ts_app_client_only_getting_started_curl_health` は ja ドキュメントの存在・必須フレーズを検証。実 UI は nene2-js-FT で別途 FT。
