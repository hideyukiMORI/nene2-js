# FT239: problem HTTP 405 on createNote
...
problem HTTP 405 on createNote. Handler `bulk_err_405_create` (fixture, category `errors-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT239"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT239 | 1     | PASS (automated) |
...
- Scenario `bulk_err_405_create` completed under Vitest.
- Persona A path: POST failure 405
