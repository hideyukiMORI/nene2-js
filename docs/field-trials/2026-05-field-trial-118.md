...
tags offset zero. Handler `q_tags_offset` (fixture, category `query`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT118"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT118 | 1     | PASS (automated) |
...
- Scenario `q_tags_offset` completed under Vitest.
- Persona A path: First page
...
### O1: 自動シナリオ `q_tags_offset`

カタログ `query` の回帰として Vitest で再現。追加の手動観察は nene2-js-FT アプリ FT で実施。
