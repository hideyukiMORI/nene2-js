...
content-type variant 1. Handler `bulk_ct_1` (fixture, category `content-type-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT480"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT480 | 1     | PASS (automated) |
...
- Scenario `bulk_ct_1` completed under Vitest.
