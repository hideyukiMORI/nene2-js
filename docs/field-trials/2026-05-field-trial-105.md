...
byField. Handler `vdx_by_field` (fixture, category `validation-dx`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT105"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT105 | 1     | PASS (automated) |
...
- Persona A path: Input highlight
