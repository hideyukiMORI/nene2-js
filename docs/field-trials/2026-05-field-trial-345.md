# FT345: tags limit+offset combo 16
...
tags limit+offset combo 16. Handler `bulk_q_tags_combo_16` (fixture, category `query-bulk`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT345"
...
| Suite               | Tests | Result           |
| ------------------- | ----- | ---------------- |
| `ft-marathon` FT345 | 1     | PASS (automated) |
...
- Scenario `bulk_q_tags_combo_16` completed under Vitest.
...
### O1: 自動シナリオ `bulk_q_tags_combo_16`
