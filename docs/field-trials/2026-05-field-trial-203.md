# FT203: [ts_app] client: health_wrong_service
...
[ts_app] client: health_wrong_service. Handler `onb_client_ts_app_health_wrong_service` (fixture, category `docs-client-onboarding`). Fixture / docs — `npm run test:ft-marathon` (no Docker required).
...
| Handler | `onb_client_ts_app_health_wrong_service` |
| Mode | fixture |
...
```text
cd nene2-js
npm run test:ft-marathon -- -t "FT203"
...
| Suite | Tests | Result |
| ----- | ----- | ------ |
| `ft-marathon` FT203 | 1 | PASS (automated) |
...
- Scenario `onb_client_ts_app_health_wrong_service` completed under Vitest.
